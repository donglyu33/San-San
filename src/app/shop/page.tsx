  ...
  return (
    <main className="min-h-screen bg-[#f6f0e6]">
      ...
      <section className="mx-auto max-w-7xl px-6 py-14 sm:px-8">
        {items.length ? <div className="grid gap-8 sm:grid-cols-2 lg:grid-cols-3">{items.map(item => <MenuItemCard key={item.id} id={item.id} {...displaySauce(item.name, item.description)} priceCents={item.priceCents} imageUrl={imageForSauce(item.name, item.imageUrl)} />)}</div> : <div className="py-20 text-center"><p className="font-display text-2xl">Our next batch is coming soon.</p><Link href="/" className="mt-6 inline-block text-xs uppercase tracking-[.2em] text-brand">Back home →</Link></div>}
      </section>
    </main>
  );
}
